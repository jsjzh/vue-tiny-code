...
export default class Color {
  ...
  getColors(type, color) {
    return this[type]
      .apply(this, Array.prototype.slice(null, arguments))
      .reduce((sum, cor) => [...sum, color.replace(/hsl\(\s*\d+\s*/, `hsl(${cor}`)], [])
  }
  ...
  getColorNumber(color, flag = true) {
    return color
      .split(/[hsl\(,rgb\(,\,\s+,,\)]/i)
      .reduce((sum, item) => (item ? [...sum, flag ? (item.match('%') ? item.slice(0, -1) : item) : item] : sum), [])
  }
  ...
  _similar(number = 1) {
    ...
    while (number) {
      arr.push(
        this._hue + chunkAngle * number > 360 ? this._hue + chunkAngle * number - 360 : this._hue + chunkAngle * number
      )
      arr.unshift(
        this._hue - chunkAngle * number < 0 ? this._hue - chunkAngle * number + 360 : this._hue - chunkAngle * number
      )
      number--
    }
    return arr
  ...
  _complement(flag = true) {
    ...
    return [
      this._hue,
      flag
        ? this._hue + chunkAngle * 2 > 360
          ? this._hue + chunkAngle * 2 - 360
          : this._hue + chunkAngle * 2
        : this._hue - chunkAngle * 2 < 0
        ? 360 + this._hue - chunkAngle * 2
        : this._hue - chunkAngle * 2,
      this._hue + chunkAngle + 180 > 360 ? this._hue + chunkAngle - 180 : this._hue + chunkAngle + 180
    ]
  }
  ...
  _doubleComplement(flag = true) {
    ...
    return [
      ...
      flag
        ...
        : this._hue - 180,
      flag
        ? this._hue + chunkAngle * 2 + 180 > 360
          ? this._hue + chunkAngle * 2 - 180
          : this._hue + chunkAngle * 2 + 180
        : this._hue + chunkAngle * 2 - 180 < 0
        ? this._hue + chunkAngle * 2 + 180
        : this._hue + chunkAngle * 2 - 180
    ]
  }
